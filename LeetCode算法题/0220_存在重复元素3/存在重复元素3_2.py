class Solution(object):
    def containsNearbyAlmostDuplicate(self, nums, k, t):
        ...
        length = len(nums)
        # TODO:红黑数插入节点第一个元素
        rb_tree.add_node(RBNode(nums[0]))
        i = 0
        for j in range(1, length):
            add_num = nums[j]
            if j - i <= k:
                # TODO: 查找判断最小绝对值，如果满足
                if rb_tree.check_node_add_num_abs(add_num, t):
                    return True
                # TODO:直接加入就好了
                rb_tree.add_node(RBNode(add_num))
            else:
                delete_num = nums[i]
                # TODO:删除第i个节点
                rb_tree.delete_node(delete_num)
                i += 1
                # TODO:查找判断最小绝对值
                if rb_tree.check_node_add_num_abs(add_num, t):
                    return True
...
class RBTree:
    ...
    def check_node_add_num_abs(self, num, abs_num):
        '''
        根据值查询节点信息
        :param val:
        :return:
