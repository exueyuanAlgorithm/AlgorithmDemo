...
tf.app.flags.DEFINE_string(flag_name="checkpoint_dir",
                           default_value="./models_bn",
                           docstring="给定模型存储的文件夹，默认为./models_bn")
tf.app.flags.DEFINE_string(flag_name="logdir",
                           default_value="./graph_bn",
                           docstring="给定模型日志存储的路径，默认为./graph_bn")
...
def create_model(input_x, is_training):
    ...
    with tf.variable_scope("net",
        ...
        with tf.variable_scope("Conv1"):
            ...
            shape = net.get_shape()
            print("归一化之前:{}".format(shape))

            # 加入批归一化
            net = batch_normalization(net, shape, is_training)
            shape = net.get_shape()
            print("归一化之后:{}".format(shape))
        ...
        with tf.variable_scope("Relu1"):
            ...
            for k in range(shape[-1]):
                image_tensor = tf.reshape(net[:, :, :, k], shape=[-1, shape[1], shape[2], 1])
                tf.summary.image(name='image', tensor=image_tensor, max_outputs=5)
        with tf.variable_scope("Poo-ling1"):
            """
...
def create_loss(labels, logits, regularization_rate=0.0):
    """
    ...
    :param regularization_rate: 正则化项系数
    :return:
    """
